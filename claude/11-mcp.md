...
配置 MCP 服务器以使用外部工具扩展您的 agent。涵盖传输类型、大型工具集的工具搜索、身份验证和错误处理。

---

[Model Context Protocol (MCP)](https://modelcontextprotocol.io/docs/getting-started/intro) 是一个用于将 AI agent 连接到外部工具和数据源的开放标准。通过 MCP，您的 agent 可以查询数据库、集成 Slack 和 GitHub 等 API，以及连接其他服务，而无需编写自定义工具实现。

MCP 服务器可以作为本地进程运行、通过 HTTP 连接，或直接在您的 SDK 应用程序中执行。

## 快速开始

此示例使用 [HTTP 传输](#httpsse-服务器)连接到 Claude Code 文档 MCP 服务器，并使用带通配符的 `allowedTools` 来允许服务器的所有工具。
...
agent 连接到文档服务器，搜索有关 hook 的信息，并返回结果。
...
- `permissionMode: "bypassPermissions"`：跳过所有安全提示，包括文件删除或运行 shell 命令等破坏性操作。请谨慎使用，尤其是在生产环境中。此模式会传播到 Task 工具生成的子 agent。
...
有关权限模式的更多详细信息，请参阅[权限](5-configuring-permissions.md)。
...
MCP 服务器使用不同的传输协议与您的 agent 通信。请查看服务器的文档以了解它支持哪种传输方式：
...
> **注意**：工具搜索需要支持 `tool_reference` 块的模型：Sonnet 4 及更高版本，或 Opus 4 及更高版本。Haiku 模型不支持工具搜索。
...
此示例使用 [Postgres MCP 服务器](https://github.com/modelcontextprotocol/servers/tree/main/src/postgres)查询数据库。连接字符串作为参数传递给服务器。agent 会自动发现数据库架构、编写 SQL 查询并返回结果：
...
SDK 在每次查询开始时会发出一个子类型为 `init` 的 `system` 消息。此消息包含每个 MCP 服务器的连接状态。检查 `status` 字段以在 agent 开始工作之前检测连接失败：
...
如果 Claude 看到工具但不使用它们，请检查您是否已通过 `allowedTools` 或更改权限模式授予了权限：
...
- **[权限](5-configuring-permissions.md)**：使用 `allowedTools` 和 `disallowedTools` 控制 agent 可以使用哪些 MCP 工具
- **[MCP 服务器目录](https://github.com/modelcontextprotocol/servers)**：浏览可用的数据库、API 等 MCP 服务器
- **[MCP 官方文档](https://modelcontextprotocol.io/docs/getting-started/intro)**：了解 Model Context Protocol 标准
