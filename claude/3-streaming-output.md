...
> **提示**：本页介绍输出流式传输（实时接收 token）。有关输入模式（如何发送消息），请参阅[向agent发送消息](/docs/zh-CN/agent-sdk/streaming-vs-single-mode)。您还可以[通过 CLI 使用 Agent SDK 流式传输响应](https://code.claude.com/docs/en/headless)。
...
启用partial messages消息后，您会收到包装在对象中的原始 Claude API 流式事件。该类型在每个 SDK 中有不同的名称：
...
class StreamEvent:
    ...
    parent_tool_use_id: str | None # 如果来自子agent，则为父工具 ID
```
...
未启用partial messages时（Python 中的 `include_partial_messages`，TypeScript 中的 `includePartialMessages`），您会收到除 `StreamEvent` 之外的所有消息类型。常见类型包括 `SystemMessage`（会话初始化）、`AssistantMessage`（完整响应）、`ResultMessage`（最终结果）和 `CompactBoundaryMessage`（指示对话历史已被压缩）。
...
此示例将文本和工具流式传输组合成一个统一的 UI。它跟踪agent当前是否正在执行工具（使用 `in_tool` 标志），以在工具运行时显示状态指示器，如 `[Using Read...]`。不在工具中时文本正常流式传输，工具完成时触发"done"消息。此模式适用于需要在多步骤agent任务期间显示进度的聊天界面。
...
- **[结构化输出](/docs/zh-CN/agent-sdk/structured-outputs)**：从agent获取类型化的 JSON 响应
- **[权限](/docs/zh-CN/agent-sdk/permissions)**：控制agent可以使用哪些工具
