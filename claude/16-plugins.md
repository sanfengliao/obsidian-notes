...
通过 Agent SDK 加载自定义插件，使用命令、agent、技能和 hook 来扩展 Claude Code

---

插件允许您使用可在项目间共享的自定义功能来扩展 Claude Code。通过 Agent SDK，您可以以编程方式从本地目录加载插件，以便向 agent 会话添加自定义斜杠命令、agent、技能、hook 和 MCP 服务器。
...
- **命令**：自定义斜杠命令
- **agent**：用于特定任务的专门子 agent
- **技能**：Claude 自主使用的模型调用功能
- **hook**：响应工具使用和其他事件的事件处理程序
...
> **Note**：路径应指向插件的根目录（包含 `.claude-plugin/plugin.json` 的目录）。
...
> **Note**：如果您通过 CLI 安装了插件（例如，`/plugin install my-plugin@marketplace`），您仍然可以通过提供其安装路径在 SDK 中使用它。检查 `~/.claude/plugins/` 以查找 CLI 安装的插件。
...
- **[SDK 中的斜杠命令](14-slash-commands.md)**：在 SDK 中使用斜杠命令
- **[SDK 中的子 agent](13-subagents.md)**：使用专门的子 agent
- **[SDK 中的 Agent 技能](15-agent-skills.md)**：使用 Agent Skills
- **[hook](7-hooks.md)**：事件 hook 机制
